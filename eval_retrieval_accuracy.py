...
async def evaluate():
    ...
    # 预计算所有 expected_answer 的 embedding（批量）
    ...
    for i, case in enumerate(eval_set, 1):
        ...
        if not expected_answer or len(expected_answer.strip()) < 2:
            continue
        total_evaluable += 1
    ...
    print(f"\n[Unified Retrieval] Top-K Recall (evaluable: {total_evaluable}):")
    for k in [1, 3, 5]:
        accuracy = hits_at_k[k] / total_evaluable * 100 if total_evaluable else 0
        print(f"  Top-{k}: {accuracy:.1f}% ({hits_at_k[k]}/{total_evaluable})")

    if detailed_misses:
        print(f"\n[Missed Examples] Top-1 ({len(detailed_misses)} total, showing first 5):")
        for i, fail in enumerate(detailed_misses[:5]):
            print(f"  {i + 1}. {fail['question'][:60]}...")
            print(f"     Answer:  {fail['answer'][:60]}")
            print(f"     Max sim: {fail['max_sim']}")
            print(f"     Top-1:   {fail['top1_preview'][:120]}...")
