...
class ChineseNumberConverter:
    ...
    def _init_mapping(self):
        """从配置加载映射表（带 fallback 默认值）"""
        self._mapping = getattr(settings, 'chinese_number_mapping', {}) or {}
        self._enable_dynamic = getattr(settings, 'enable_dynamic_conversion', True)
        self._enable_digits = getattr(settings, 'enable_digits', True)
        self._enable_units = getattr(settings, 'enable_units', True)
