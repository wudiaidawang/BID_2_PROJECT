...
class ParentContextExpander:
    ...
    从向量库查询其 parent chunk，附加完整法条内容 (parent_content)，
    按 article_id 去重。
    """
    ...
    def _query_missing(self, missing_ids: Set[str], collection: str):
        """查询缺失的 parent（ChromaDB 用 raw collection get，Milvus 用 get_by_ids）"""
        try:
            if self._backend == "milvus":
                ...
                docs = store.get_by_ids(collection, list(missing_ids))
                cache = self._parent_cache.setdefault(collection, {})
                for doc in docs:
                    cache[doc["id"]] = doc
            else:
                from app.data.storage.chroma_store import ChromaStore
                store = self._store if isinstance(self._store, ChromaStore) else ChromaStore()
                col = store.get_raw_collection(collection)
