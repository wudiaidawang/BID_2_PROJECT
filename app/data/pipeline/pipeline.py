...
from app.data.pipeline.preprocessor import QueryPreprocessor
from app.data.pipeline.retrievers import VectorRetriever, BM25Retriever
from app.data.pipeline.fusion import RRFFusion, WeightedFusion
from app.data.pipeline.expanders import ParentContextExpander, NoopExpander
from app.data.pipeline.rerankers import BgeReranker
...
class SearchPipeline:
    ...
    def __init__(self):
        ...
        self.bm25 = BM25Retriever(self._store)
    ...
    def _retrieve_and_fuse(self, normalized: str, original: str,
        ...
        # Milvus: BM25 由服务端内置，无需传文档列表；ChromaDB: 需加载全部文档建索引
        if settings.vector_store_backend == "milvus":
            bm25_results = self.bm25.search(normalized, collection, None,
