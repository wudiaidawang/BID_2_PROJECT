# -*- coding: utf-8 -*-
"""LangGraph 路由状态机 — fast/think 双模式共享同一 Graph

## fast 模式 (零规划开销)
    quick_intercept
      ├─ greeting/unrelated → END (direct)
      └─ otherwise → binary_classify → END (SQL / RAG)

## think 模式 (TaskAnalysis → 智能分流)
    quick_intercept
      ├─ greeting/unrelated → END (direct)
      └─ otherwise → task_analysis
                        ├─ 0 task / low conf → binary_classify → END
                        ├─ 1 task → binary_classify → END
                        └─ 2+ tasks → tool_planning → END (Planner DAG)

两条路线通过 router_mode 条件边切换，共用 quick_intercept + binary_classify 节点。
"""
...
class RouterState(TypedDict, total=False):
    ...
    session_manager: Any
    llm: Any
    router_mode: str                 # "fast" | "think" — 控制走哪条分支
    # Intercept outputs
...
def _tool_planning_node(state: RouterState) -> dict:
    ...
    tools_description = (
        "rag_search: RAG混合检索（法规条文 + 招标项目案例 + 法条精确查询）。"
        "参数: query(自然语言), collection(unified/regulations/bids)\n"
        "sql_search: SQL统计查询（数量/金额/排名等结构化数据）。参数: query(自然语言统计问题)"
    )
...
def _fallback_plan(question: str, reason: str) -> dict:
    return {
        ...
        "plan": [{"step": 1, "task_id": "t1", "tool": "rag_search",
...
def _route_after_intercept(state: RouterState) -> str:
    ...
    router_mode = state.get("router_mode", "think")
    if router_mode == "fast":
        # fast 模式: 跳过 TaskAnalysis，直接 BinaryClassify 判 SQL vs RAG
        return "binary_classify"

    # think 模式: 进入 TaskAnalysis → 按 task 数分流
...
def build_router_graph() -> CompiledStateGraph:
    """构建双模路由 Graph — fast/think 共享节点，条件边分流"""
    graph = StateGraph(RouterState)

    # Add nodes (所有节点都被两种模式共用)
    ...
    # 条件边: quick_intercept → direct(END) / binary_classify(fast) / task_analysis(think)
    ...
    # 条件边: task_analysis → binary_classify(简单) / tool_planning(复杂, Planner DAG)
    graph.add_conditional_edges(
        "task_analysis",
...
class LangGraphRouter:
    """LangGraph 双模路由器 — fast/think 共享同一 Graph，条件边分流

    fast:  quick_intercept → binary_classify → END  (零规划开销，直接 SQL/RAG)
    think: quick_intercept → task_analysis → binary_classify / tool_planning → END
    """

    def __init__(self, llm=None):
        self.llm = llm
        self._graph = build_router_graph()
        self._router_mode = settings.router_mode
        print(f"[LangGraphRouter] Dual-mode graph compiled (mode={self._router_mode})")
