...
# ═══════════════════════════════════════════════════════════════════════
# 统一路由工厂 — fast/think 双模共享同一 LangGraph，条件边分流
# ═══════════════════════════════════════════════════════════════════════

def create_router(llm=None):
    """创建路由实例。

    fast/think/auto/planner/binary/intent 全都走 LangGraphRouter。
    实际分支由 router_mode 在 Graph 内部的条件边决定:
      fast  → quick_intercept → binary_classify → END
      think → quick_intercept → task_analysis → binary_classify / tool_planning → END
    """
    from app.agent.router_graph import LangGraphRouter
